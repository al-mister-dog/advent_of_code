...
def sum_possible_games_ids(input)
  max_cubes = {
    'red' => 12,
    'green' => 13,
    'blue' => 14
  ...
  input.split("\n").each do |g|
    game, cubes = g.split(':')
    game_id = game.sub('Game ', '').to_i
    sets = cubes.strip.split('; ')

    possible = true

    results = sets.each do |set|
      break unless possible
      colors = set.split(', ')

      inner_results = colors.each do |color|
        break unless possible
        num, col = color.split(' ')
        possible = num.to_i <= max_cubes[col]
      end
    end
    count += game_id if possible
  end
  count
end

def sum_fewest_possible_colors(input)
  input.split("\n").sum(0) do |g|
    _game, cubes = g.split(':')
    sets = cubes.strip.split('; ')
    max = sets.reduce({
...
puts sum_fewest_possible_colors(input)
